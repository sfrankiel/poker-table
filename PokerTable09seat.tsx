...
import { SeatState, Player } from './types';

const TOTAL_POSITIONS = 9; // 9 player seats, no physical dealer seat

export interface PokerTable9PlayerProps {
...
export const PokerTable9Player: React.FC<PokerTable9PlayerProps> = ({
  initialSeats,
  ...
  const validDealerPosition = Math.max(0, Math.min(8, dealerButtonPosition));
  const dealerPosition = positions[validDealerPosition];
